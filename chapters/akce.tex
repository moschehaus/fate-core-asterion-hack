...
\begin{akce}
  ...
  \iconitem{vytvoreni-ikona}{\texttt{Vytvoření výhody:}\footnote{Pravidlové postihnutí vytváření výhody je velký posun oproti pravidlům M16.} Akce \texttt{Vytvoření výhody} je základní způsob, jak zvýšit své šance na úspěch při všech ostatních akcích. Pomocí této akce se buď dají vytvořit nové aspekty (pronést plamennou řeč a stát se \asp{Miláčkem davu}, hodit písek do očí protivníkovi a dočasně ho \asp{Oslepit}, vylepšit povoz a učinit ho tak \asp{Opancéřovaným}), objevit již existující aspekty (po chvíli zkoumání získat \asp{Usvědčující důkaz}, rozhovorem zjistit, že strážnému \asp{Jde hlavně o peníze} atd.), nebo získat volná vyvolání na již existující aspekty (připravit si nečekaný útok v místnosti, kde je \asp{Všude tma}, trochu někoho vyprovokovat a využít, že se \asp{Lehko naštve} a další). }
  \iconitem{utok-ikona}{\texttt{Útok:} Akce \texttt{Útok} je přímočará - způsobit nepříteli zranění, fyzické či duševní. Může se jednat o seky mečem, údery a kopy, střelbu, nebo např. pokus někoho urazit, rozlítostnit, deprimovat, rozhodit. }
  \iconitem{obrana-ikona}{\texttt{Obrana:} Po akci \texttt{Obrana} sáhne hráč vždy, když chce jiným postavám ztěžovat pokusy vykonávat svoje akce \texttt{Vytvoření výhody i Útok}. Všimněte si, že bránit se akci \texttt{Překonání} technicky není možné; možné je ale tvořit aktivní opozici, jež musí ``útočník'' přehodit. \texttt{Obrana} jediná akce, kterou může postava zahrát mimo své kolo a to v libovolném množství. \footnote{Může se jevit, že je velmi silné, aby se postava mohla bránit kolikrát chce. Ve skutečnosti toto ale velmi urychluje a usnadňuje konflikty, neboť nedává prostor diskuzím o tom, zda-li dává smysl, aby se postava mohla bránit.}} 
\end{akce}

\section{Čtyři výsledky}
\label{sec:4-vysledky}

Podle toho, jak veliký je součet hod + úroveň dovednosti, nastane právě jedna z akcí níže:
...
\begin{akce}
  ...
  \iconitem{utok-ikona}{Postavě se nepodaří způsobit žádné zranění, ale získává posílení.}\footnote{Díky tomuto jsou konflikty svižnější i v případě vyrovnaných nepřátel.}
...
Za povšimnutí stojí, že výsledky akce \texttt{Obrana} zrcadlí výsledky akcí \texttt{Vytvoření výhody} a \texttt{Útok} (tj. těm, proti kterým se lze bránit). Znamená to prostě, že když obránce uspěje, útočník ne a vice versa. To je podstatné, aby nedošlo ke zvláštnímu skládání efektů.
